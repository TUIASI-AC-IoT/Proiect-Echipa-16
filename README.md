...
FILE_UPLOAD
Client → Server
Header CoAP (POST) + nume fișier + conținut fișier
FILE_DOWNLOAD
Client → Server
Header CoAP (GET) + calea fișierului
FILE_DELETE
Client → Server
Header CoAP (DELETE) + calea fișierului
FILE_MOVE
Client → Server
Header CoAP (CUSTOM/MOVE) + calea sursă + calea destinație
DIR_LIST
Client → Server
Header CoAP (GET) + calea directorului
RESPONSE_OK / ERROR
Server → Client
Header CoAP (ACK) + cod + mesaj textual

3.Operatiuni specifice CoAP si interactiunea intre server-client
-POST /upload
...
Main thread: Este responsabil doar pentru interacțiunea cu utilizatorul (preluarea input()-ului). Când primește o comandă, o adaugă într-o coadă (Queue) și revine imediat la input().
Second thread:  Este responsabil doar pentru rețea. Rulează în buclă, extrage sarcini din coadă, trimite pachete (sendto), așteaptă răspunsuri (recvfrom - blocant) și afișează rezultatul.
